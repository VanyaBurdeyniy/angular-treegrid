...
// Copy/Paste
  ...
  ngOnInit(): void {
    let me = this;
    this.http.get('http://185.233.117.219:9000/columns').subscribe(data => {
      me.columns = data;
    });
    this.dataManager = new DataManager({
      url:
          'http://185.233.117.219:9000/file',
  ...
  showDialog (headerName: string, placeholder: string, isEdit: boolean): void {
    ...
    function dlgButtonClick(): void {
      ...
      me.columns = [];
      me.http.post('http://185.233.117.219:9000/columns',
  ...
  contextMenuClick (args?: MenuEventArgs): void {
    switch (args.item.id) {
      ...
      case 'deleteColumn':
        // @ts-ignore
        this.http.post('http://185.233.117.219:9000/deleteColumn', {field: this.selectedColumn.field}).subscribe(data => {
          this.columns = data;
        });
